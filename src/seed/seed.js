...
async function createMatch(seedMatch) {
  ...
  // API returns { message, match }, while other endpoints return { data }.
  const match = responsePayload.match ?? responsePayload.data ?? responsePayload;
  if (!match || typeof match !== "object") {
    throw new Error("Unexpected createMatch response shape.");
  }
  return match;
}
